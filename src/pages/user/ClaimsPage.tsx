...
export default function ClaimsPage() {
  ...
  const [drawerReceipts, setDrawerReceipts] = useState<Receipt[]>([]);
  const uploadingClaimId = useRef<string | null>(null);
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        const approveLike =
          ru === "APPROVER" || ru === "FINANCE" || ru === "TENANT_ADMIN" || ru === "MANAGER" || ru === "HR";
  ...
  const openReceiptPreview = async (r: Receipt) => {
  ...
  const loadDrawerReceipts = async (claimId: string) => {
    try {
      const res = await api.get(`/api/claims/${claimId}/receipts`);
      setDrawerReceipts(res.data.items || []);
    } catch {
      setDrawerReceipts([]);
    }
  };

  const uploadPendingReceipts = async (claimId: string) => {
    for (const f of pendingFiles) {
      if (!f.originFileObj) continue;
      const fd = new FormData();
      fd.append("file", f.originFileObj);
      try {
        await api.post(`/api/claims/${claimId}/receipts`, fd, {
          headers: { "Content-Type": "multipart/form-data" },
        });
      } catch (e: any) {
        message.warning(`Receipt "${f.name}" upload failed: ${e?.response?.data?.error || e.message}`);
      }
    }
    setPendingFiles([]);
  };
  ...
  const create = async () => {
    ...
    try {
      ...
      const claimRes = await api.post("/api/claims",payload);
      const claimID = claimRes.data.id;
      if (pendingFiles.length > 0) await uploadPendingReceipts(claimID);
  ...
  const updateClaim = async () => {
    ...
    try {
      ...
      if (pendingFiles.length > 0) await uploadPendingReceipts(editingClaimId);
    ...
    <div style={{ padding: pagePad }}>
      <div style={{ maxWidth: 1280, margin: "0 auto" }}>
                  ...
                  {canEditDraft ? (
                    <Upload
                      ...
                      beforeUpload={async (file) => {
                        ...
                        fd.append("file", file);
                        try {
                          await api.post(`/api/claims/${selectedClaim!.id}/receipts`, fd, {
                            headers: { "Content-Type": "multipart/form-data" },
                          });
                          message.success(`${file.name} uploaded`);
                          await loadDrawerReceipts(selectedClaim!.id);
                        } catch (e: any) {
                    ...
                    >
                      <Button size="small" icon={<UploadOutlined />}>Upload Receipt</Button>
                    </Upload>
                  ) : null}
                </Space>
                {drawerReceipts.length === 0 ? (
                  <Text type="secondary">No receipts attached.</Text>
                ) : (
                  <Table
                    size="small"
                    pagination={false}
                          ...
                          <Space>
                            ...
                            {canEditDraft ? (
                              <Popconfirm
                                ...
                                onConfirm={async () => {
                                  try {
                                    await api.delete(`/api/claims/${selectedClaim!.id}/receipts/${r.id}`);
        ...
        >
          ...
          {/* Receipt upload — common to all forms */}
          <Card title="Receipts" size="small" style={{ marginTop: 12 }} bodyStyle={{ padding: screens.xs ? 12 : 16 }}>
            <Upload
              accept="image/*,.pdf"
              multiple
              fileList={pendingFiles}
              beforeUpload={(file) => {
                setPendingFiles((prev) => [...prev, { uid: file.uid, name: file.name, originFileObj: file } as UploadFile]);
                return false;
              }}
              onRemove={(file) => setPendingFiles((prev) => prev.filter((f) => f.uid !== file.uid))}
            >
              <Button icon={<UploadOutlined />}>Attach Receipts (JPG / PNG / PDF, max 10 MB each)</Button>
            </Upload>
            {pendingFiles.length > 0 ? (
              <Text type="secondary" style={{ marginTop: 4, display: "block" }}>
                {pendingFiles.length} file(s) will be uploaded when you save.
              </Text>
            ) : null}
          </Card>
                    ...
                    return (
                      <Col key={key} {...itemColProps}>
                        <Form.Item name={key} label={f.label || key}>
                          <Input disabled placeholder="File upload (MVP placeholder)" />
                        </Form.Item>
                      </Col>
                    );
                  }
